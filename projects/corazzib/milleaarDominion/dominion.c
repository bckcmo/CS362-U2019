...
int ambassadorEffect(int choice1, int choice2, struct gameState *state, int handPos){
  ...
  for (int i = 0; i < state->numPlayers; i++){
      printf("supply count before on iteration %i = %i\n", i, state->supplyCount[state->hand[currentPlayer][choice1]]);
	    if (i != currentPlayer){
	        gainCard(state->hand[currentPlayer][choice1], state, 0, i);
	    }
      state->supplyCount[state->hand[currentPlayer][choice1]]--;
      printf("Supply count after on iteration %i = %i\n", i, state->supplyCount[state->hand[currentPlayer][choice1]]);
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case baron:
      ...
      return baronEffect(choice1, state);
