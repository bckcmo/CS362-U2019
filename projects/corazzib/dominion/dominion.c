...
int playTribute(int currentPlayer, int nextPlayer, struct gameState *state)
{
  ...
  for(i = 0; i <= 2; i ++) {
    ...
    if(isType(card, treasure_card)) {//Treasure cards
      state->coins += 1;
    } else if (isType(card, victory_card)) {//Victory Card Found
      drawCard(currentPlayer, state);
      drawCard(currentPlayer, state);
    } else {//Action Card
...
int isType(int card, int type)
{
  switch(type) {
    case curse_card:
      return (card == curse);
    case victory_card:
      return ((card <= province && card >= estate) || card == gardens || card == great_hall);
    case treasure_card:
      return (card <= gold && card >= copper);
    case action_card:
      return ((card <= treasure_map && card >= adventurer) && card != gardens && card != great_hall);
    case any_card:
      return (card <= treasure_map && card >= curse);
  }
  return -1;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
  {
    ...
    case mine:
      return playMine(currentPlayer, choice1, choice2, handPos, state);
    ...
    case minion:
      return playMinion(currentPlayer, choice1, choice2, handPos, state);
    ...
    case tribute:
      return playTribute(currentPlayer, nextPlayer, state);

    case ambassador:
      return playAmbassador(currentPlayer, choice1, choice2, handPos, state);
